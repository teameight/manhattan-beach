...
$(window).on("load", function() {
	...
	function setWaterHeight() {
		...
		var offsetMultiplier = 11;
		if (offset < windowW) offsetMultiplier = 12;

		var $vert_offset = (offset / windowW) * offsetMultiplier; //$v_o_factor * 4; // half the ((height/width)-overlap) of each element
		var $translate_z = $persp * ( 1 - ( $persp / $vert_offset ) );
		var $scale = ( $persp - $translate_z ) / $persp;


		$('.wave-wrap').css({
			'height': (((offset / windowW) * 100) + 100 + ((windowW / windowH)*200)-($wave_reveal*4)) + 'vw'
		});
